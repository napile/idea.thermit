...
import org.napile.idea.thermit.config.impl.TargetChooserDialog;
import org.napile.idea.thermit.config.impl.configuration.UIPropertyBinding;
...
public class AntArtifactPropertiesEditor extends ArtifactPropertiesEditor
{
	...
	{
		if(myProperties.isEnabled() != myRunTargetCheckBox.isSelected())
			return true;
		if(myTarget == null)
		{
			return myProperties.getFileUrl() != null;
		}
		if(!Comparing.equal(myTarget.getName(), myProperties.getTargetName()))
			return true;

		final VirtualFile file = myTarget.getModel().getBuildFile().getVirtualFile();
		if(file != null && !Comparing.equal(file.getUrl(), myProperties.getFileUrl()))
			return true;
	...
	public void reset()
	{
		...
		myTarget = myProperties.findTarget(ThermitConfiguration.getInstance(myContext.getProject()));
