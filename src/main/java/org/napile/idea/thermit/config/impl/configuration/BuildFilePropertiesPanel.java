...
import org.napile.idea.thermit.config.impl.TargetFilter;
...
public class BuildFilePropertiesPanel
{
	...
	private static class Form implements Disposable
	{
		...
		private Form(@NotNull final Project project)
		{
			myTabs = new Tab[]{
					new PropertiesTab(),
					new ExecutionTab(GlobalThermitConfiguration.getInstance(), project),
	...
	private static class PropertiesTab extends Tab
	{
		...
		public PropertiesTab()
		{
			...
			UIPropertyBinding.TableListBinding<BuildFileProperty> tableListBinding = getBinding().bindList(myPropertiesTable, PROPERTY_COLUMNS, AntBuildFileImpl.ANT_PROPERTIES);
			tableListBinding.setColumnWidths(GlobalThermitConfiguration.PROPERTIES_TABLE_LAYOUT);
	...
	private static class FiltersTab extends Tab
	{
		...
		public FiltersTab()
		{
			...
			tableListBinding.setColumnWidths(GlobalThermitConfiguration.FILTERS_TABLE_LAYOUT);
	...
	static class ExecutionTab extends Tab
	{
		...
		private final GlobalThermitConfiguration myAntGlobalConfiguration;
		private final Project myProject;

		public ExecutionTab(final GlobalThermitConfiguration antConfiguration, @NotNull final Project project)
		{
		...
		public void reset(AbstractProperty.AbstractPropertyContainer options)
		{
			String projectJdkName = ThermitConfigurationImpl.DEFAULT_JDK_NAME.get(options);
			myJDKsController.setRenderer(new AntUIUtil.ProjectJdkRenderer(true, projectJdkName));
			super.reset(options);
			myJDKsController.resetList(null);
			myProjectDefaultAnt = ThermitConfigurationImpl.DEFAULT_ANT.get(options);
			updateDefaultAnt();
		}
		...
		public void apply(AbstractProperty.AbstractPropertyContainer options)
		{
			ThermitConfigurationImpl.DEFAULT_ANT.set(options, myProjectDefaultAnt);
			super.apply(options);
		}
