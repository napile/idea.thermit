...
public class EditPropertyContainer extends AbstractProperty.AbstractPropertyContainer
{
	...
	{
		if(myModifications.containsKey(property))
			return myModifications.get(property);
	...
	{
		if(myParent.hasProperty(property))
			delegateSet(myParent, property, value);
		else
			myModifications.put(property, value);
	}
	...
	public void apply()
	{
		for(AbstractProperty property : myModifications.keySet())
		{
			AbstractProperty.AbstractPropertyContainer container = findContainerOf(property);
			if(container != null)
				delegateSet(container, property, myModifications.get(property));
		}
